...
export async function findByEmail(email: IUser["email"]) {
  const result = await sql<IUser[]>`SELECT * FROM users WHERE email = ${email}`;
  return result[0] || null;
}

export async function findById(userId: IUser["id"]) {
...
export async function updateGoogleId(email: string, googleId: string) {
  return await sql`UPDATE users SET google_id = ${googleId} WHERE email = ${email}`;
}

export async function clearOTP(userId: IUser["id"]) {
